...
(* Test case: Verifies that a file can be added to a directory
   - Creates a root directory
   - Adds a file with content
   - Verifies the file exists in the tree
   - Checks that the file content matches *)
let test_add_file () =
...
(* Test case: Verifies directory lookup functionality
   - Creates a root directory with a subdirectory
   - Attempts to find the subdirectory by path
   - Verifies the directory name matches *)
...
(* Test case: Verifies behavior when searching for non-existent directory
   - Creates a root directory
   - Attempts to find a directory that doesn't exist
...
(* Test case: Tests complex file structure operations
   ...
   root/
     └── sub1/
         └── sub2/
             └── test.txt
   Then verifies:
   1. The existence of sub2 directory
   2. The content of test.txt *)
...
(* Test case: Verifies node removal functionality
   - Creates a root directory with multiple files and subdirectories
   - Removes a specific file
   - Verifies:
     1. The removed file no longer exists
     2. All other nodes remain intact
     3. The tree structure is preserved *)
