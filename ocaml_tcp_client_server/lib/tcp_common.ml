...
(** TCP Common module provides utility functions for TCP socket operations *)
...
let safe_close (socket_fd : Unix.file_descr) : unit Lwt.t =
  ...
  Lwt.catch
    (fun () ->
      ...
      let handle_shutdown_error = function
        | Unix.Unix_error ((Unix.ENOTCONN | Unix.EBADF), _, _) ->
            Logs_lwt.info (fun m -> m "Socket was not connected or already closed")
        | exn ->
            Logs_lwt.err (fun m ->
      ...
      Lwt.catch
        (fun () ->
          ...
          Logs_lwt.info (fun m -> m "Socket shutdown complete."))
      ...
      try
        ...
        Logs_lwt.info (fun m -> m "Socket closed successfully.")
      with
