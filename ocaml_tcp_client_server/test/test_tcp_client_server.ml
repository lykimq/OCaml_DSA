(** TCP Client-Server Test Suite
    This module contains integration tests for the TCP client-server implementation.

    The test suite covers:
    - Basic connectivity
    - Message transmission and verification
    - Client capacity limits
    - Message type handling
    - Client reconnection scenarios

    Each test uses a different port to ensure isolation.
    All tests use localhost (127.0.0.1) for communication.
*)
...
(** Tests basic client-server connection establishment and teardown.
    - Starts server
    - Connects client after delay
    - Verifies connection
...
(** Tests message transmission with signature verification.
    - Verifies message encoding/decoding
    - Tests digital signature functionality
    - Validates server acknowledgment
...
(** Helper function to create multiple test clients
    @param n Number of clients to create
...
(** Tests server behavior with maximum allowed clients.
    Verifies that the server:
    - Accepts up to max_clients connections
    - Handles multiple concurrent clients
    - Processes messages from all clients
*)
...
(** Tests server behavior when exceeding maximum client limit.
    Verifies that the server:
    - Rejects connections beyond max_clients
    - Maintains stability under connection pressure
    - Continues serving existing clients
*)
...
(** Tests handling of different message types.
    Verifies server processing of:
    - Request messages
    - Info messages
    - Critical messages
    - Warning messages
...
(** Tests client reconnection scenarios.
    Verifies:
    - Clients can disconnect and reconnect
    - Server handles reconnections properly
    - Message processing works after reconnection
*)
...
(** Test suite definition combining all test cases *)
let tests =
  [
